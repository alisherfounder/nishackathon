...
import { ScatterplotLayer, PathLayer, type Layer } from "deck.gl";

const API      = process.env.NEXT_PUBLIC_API ?? "/api";
const MAPS_KEY = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY ?? "";

// A clean, minimal Google Maps style for the citizens view
const MAP_STYLES: google.maps.MapTypeStyle[] = [
  ...
  { featureType: "water",            elementType: "geometry", stylers: [{ color: "#c9d8e4" }] },
...
// ── Deck.GL overlay wired to the Google Maps instance ─────────────────────────
...
function DeckOverlay({
  ...
  if (!overlayRef.current) {
    overlayRef.current = new GoogleMapsOverlay({
      ...
      getCursor: ({ isHovering }: { isHovering: boolean }) =>
        pickingMode ? "crosshair" : isHovering ? "pointer" : "grab",
    });
  }
  ...
  // Sync layers and cursor on every render
  useEffect(() => {
  ...
  // Clear tooltip when layer is no longer hovered — handled inside layer onHover callbacks
  void onTooltip; // acknowledged — used by parent via layer callbacks
...
export default function CitizensMapClient() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (roadRes.status   === "fulfilled" && roadRes.value.ok) {
      const raw: NotificationItem[] = await roadRes.value.json();
      setRoads(raw.filter(r => r.geometry).map(r => ({ id: r.id, title: r.title, path: JSON.parse(r.geometry!) as [number, number][] })));
    }
  ...
  const layers: Layer[] = [
    ...(roads.length > 0 ? [new PathLayer({
      id: "roads",
      data: roads,
      getPath:  (d: RoadItem) => d.path,
      getWidth: 5,
      getColor: [249, 115, 22, 200] as [number, number, number, number],
      widthMinPixels: 3,
      capRounded: true, jointRounded: true,
      pickable: true,
      onHover: ({ object, x, y }: { object?: RoadItem; x: number; y: number }) => {
        setTooltip(object ? { x, y, label: "Road Work", title: object.title } : null);
      },
    })] : []),
    ...(geoProjects.length > 0 ? [new ScatterplotLayer({
      id: "projects",
      data: geoProjects,
      ...
      onHover: ({ object, x, y }: { object?: Project; x: number; y: number }) => {
        setTooltip(object ? { x, y, label: `${object.institution} · ${object.status}`, title: object.title, description: object.description ? truncateWords(object.description) : undefined } : null);
      },
    })] : []),
    ...(geoJams.length > 0 ? [new ScatterplotLayer({
      id: "traffic",
      data: geoJams,
      ...
      onHover: ({ object, x, y }: { object?: NotificationItem; x: number; y: number }) => {
        setTooltip(object ? { x, y, label: "Traffic Jam", title: object.title, description: object.body ? truncateWords(object.body) : undefined } : null);
      },
    })] : []),
    ...(geoDangers.length > 0 ? [new ScatterplotLayer({
      id: "danger",
      data: geoDangers,
      getPosition:  (d: NotificationItem) => [d.lon!, d.lat!],
      getRadius:    70,
      ...
      onHover: ({ object, x, y }: { object?: NotificationItem; x: number; y: number }) => {
        setTooltip(object ? { x, y, label: "Danger Zone", title: object.title, description: object.body ? truncateWords(object.body) : undefined } : null);
      },
    })] : []),
    ...(geoReports.length > 0 ? [new ScatterplotLayer({
      id: "reports",
      data: geoReports,
      getPosition:  (d: ReportItem) => [d.lon!, d.lat!],
      getRadius:    55,
      getFillColor: [147, 51, 234, 200] as [number, number, number, number],
      getLineColor: [147, 51, 234, 255] as [number, number, number, number],
      lineWidthMinPixels: 2,
      stroked: true, pickable: true,
      radiusMinPixels: 6, radiusMaxPixels: 16,
      onHover: ({ object, x, y }: { object?: ReportItem; x: number; y: number }) => {
        setTooltip(object ? { x, y, label: `Report · ${object.status}`, title: object.title, description: object.description ? truncateWords(object.description) : undefined } : null);
      },
    })] : []),
  ];

  const handleMapClick = useCallback((e: MapMouseEvent) => {
    if (!pickingMode || !e.detail.latLng) return;
    setClickedCoords({ lat: e.detail.latLng.lat, lon: e.detail.latLng.lng });
    setShowReport(true);
  }, [pickingMode]);

  async function handleReportSubmit(ev: React.FormEvent) {
    ev.preventDefault();
  ...
  return (
    <APIProvider apiKey={MAPS_KEY}>
      <div className="relative" style={{ height: "calc(100vh - 120px)" }}>
        ...
        {/* Google Map */}
        <Map
          defaultCenter={{ lat: 43.55, lng: 77.05 }}
          defaultZoom={10}
          mapTypeId="roadmap"
          styles={MAP_STYLES}
        ...
        >
          <DeckOverlay layers={layers} pickingMode={pickingMode} onTooltip={setTooltip} />
        </Map>
        ...
        {pickingMode && !showReport && (
          <div className="absolute bottom-5 left-4 right-4 z-10 flex items-center justify-between gap-3 rounded-2xl bg-blue-600 px-4 py-3 shadow-lg">
            ...
            <button
              onClick={() => { setPickingMode(false); setClickedCoords(null); }}
        ...
        {tooltip && (
          <div
            className="absolute z-20 bg-white border border-gray-200 rounded-xl px-3 py-2.5 shadow-lg pointer-events-none max-w-xs"
              ...
              {submitted ? (
                ...
                <form onSubmit={handleReportSubmit} className="flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => { setShowReport(false); setPickingMode(false); setClickedCoords(null); }}
